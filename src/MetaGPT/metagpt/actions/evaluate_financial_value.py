...
Role: Your role is to evaluate the financial value a proposed innovation idea brings to a category of product for a company, using both the given context and your own knowledge base.
Requirements: Evaluate the change in financial value of all the ideas in the context with as much critical thought as possible. For each idea, you have to answer four questions; return a two sentence justification and a score on the scale of 1-9 (where 1 represents a negative impact, 5 no change, and 9 is a positive impact). Compare the idea to similar products to reason your justifications: 
    1) SALES VOLUME: What is the impact of the idea on the expected sales volume compared to existing products?
    2) RATE OF RETURN: How does the idea affect the rate of return on investment compared to existing products?
    3) PAYBACK TIME: How does the idea affect the payback time compared to existing products?
...
class EvaluateFinancialValue(Action):
    ...
    def _save(self, financial_value):
        ...
        list_of_financial_value = financial_value.instruct_content.dict()["Values"]
