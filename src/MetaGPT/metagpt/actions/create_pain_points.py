...
Role: Your role is to analyse journey maps of individuals and identify issues they experience when they are using a category of product. Try to keep the pain point to a maximium of ten words long. Once you have created each pain point point, rank each pain point on two metrics: impact (how commonly experienced is the pain point? pain points that have similar pain points in the list should be scored highly) and difficulty (how hard to fix is it likely to be, factoring in the context of company provided in the context). Each metric should be an integer score out of 100. A high score should mean larger impact and high difficulty, for the two metrics respectively.
Requirements: According to the context and using the already created personas within it, fill in the following missing information, each section name is a key in json ,If the requirements are unclear, ensure minimum viability and avoid excessive design. The response should be a MAXIMUM of 1500 tokens long.

##Project name: Repeat the project name from the context exactly. Dont change anything and don't add any characters
##Persona specfic pain points: Provided as Python Dict[str, List[str]], the key is persona name, and the value is alist of persona issues. Create 3 pain points for each persona.
...
class CreatePainPoints(Action):
    ...
    def _save(self, needs, s):
        ...
        needs_path = workspace / "imperatives"
        ...
        ranked_file = needs_path / "pain_points_ranked.txt"
    ...
    async def run(self, context, format=CONFIG.prompt_format, *args, **kwargs) -> ActionOutput:
        ...
        self._save(needs,s)
        output_string = ("Project name: {p}\nPain points to be addressed: {r}").format(p = needs.instruct_content.dict()["Project name"], r = rankings[0:3]['pain_point'])
