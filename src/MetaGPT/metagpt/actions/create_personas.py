...
Role: Your role is to create 5 customer personas that would interact with a given category of product.
Requirements: According to the context, fill in the following missing information, each section name is a key in json ,If the requirements are unclear, ensure minimum viability and avoid excessive design. Never change the section name whatsover. The response should be a MAXIMUM of 1500 tokens long.
...
##Personas: Provided as Python List[Dict[str,str]], the parameters are persona name, persona decription, respectively. The persona name should only include alphabetical characters and spaces. The name used should be a real name and consistent throughout. Don't add any new keys to the dictionary. The goals refers to the personas personal goals, not their goals regarding the category of product. Similarly, the fustrations refers to the personas personal fustrations, not their fustrations regarding the category of product.
